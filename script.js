...
const CONFIG_MEDELLIN_NORTE = {
    RUTA: "Medellín Norte",
    TABLAS_SUPABASE: {
        conductores: 'conductores_medellin_norte',
        vehiculos: 'vehiculos_medellin_norte', 
        tablas: 'tablas_medellin_norte',
        viajes: 'viajes_medellin_norte'
    },
    TIEMPOS_VIAJE: {
        "La Ceja - Medellín Norte": 110, // 1h 50min
        "Medellín Norte - La Ceja": 110
    },
    CONFIG_PUNTOS_CONTROL: {
        "La Ceja - Medellín Norte": [
            {"id": "tramo_memos", "label": "CIT-Los Memos", "duracion": 5},
            {"id": "tramo_terminal_norte", "label": "Terminal del Norte", "duracion": 105}
        ],
        "Medellín Norte - La Ceja": [
            {"id": "tramo_exposiciones", "label": "Terminal del Norte-Exposiciones", "duracion": 20},
            {"id": "tramo_ceja", "label": "Exposiciones-La Ceja", "duracion": 90}
        ]
    },
    CAUSAS_EXTERNAS: ["Accidente", "Obras Viales", "Tráfico", "Otros"]
};
...
async function cargarSelectores() {
    ...
    try {
        ...
        console.log('✅ Selectores de Medellín Norte cargados correctamente');
    } catch (error) {
        console.error('❌ Error al cargar selectores Medellín Norte:', error);
        alert('Error al cargar los datos desde Supabase. Revisa la consola.');
    }
}
// ===== FUNCIONES DE PUNTOS DE CONTROL ===== // pendiente del cambio por comfirmacion. 
function actualizarPuntosControl(ruta) {
    ...
    if (!ruta || !CONFIG_MEDELLIN_NORTE.CONFIG_PUNTOS_CONTROL[ruta]) return;

    const puntos = CONFIG_MEDELLIN_NORTE.CONFIG_PUNTOS_CONTROL[ruta];
            ...
            horaRealInput.addEventListener('change', function() {
                ...
                if (proyeccionSpan && proyeccionSpan.dataset.minutos) {
                    ...
                    if (!esUltimo) {
                        ...
                        const maxTiempo = CONFIG_MEDELLIN_NORTE.TIEMPOS_VIAJE[ruta];
...
function calcularProyecciones(ruta, salidaReal) {
    ...
    if (!ruta || !salidaReal || !CONFIG_MEDELLIN_NORTE.CONFIG_PUNTOS_CONTROL[ruta]) {
        console.error('DEBUG RUTA INVERSA: Ruta, salida o config inválida:', ruta, salidaReal, CONFIG_MEDELLIN_NORTE.CONFIG_PUNTOS_CONTROL[ruta]); // DEBUG
        return;
    }
    const puntos = CONFIG_MEDELLIN_NORTE.CONFIG_PUNTOS_CONTROL[ruta];
...
function validarLlegadaFinal(ruta) {
    ...
    if (!horaRealUltimo || !horaRealUltimo.value) {
        alert(`La hora real de llegada final (${CONFIG_MEDELLIN_NORTE.CONFIG_PUNTOS_CONTROL[ruta][CONFIG_MEDELLIN_NORTE.CONFIG_PUNTOS_CONTROL[ruta].length - 1].label}) es obligatoria.`);
        return false;
    }
    return true;
}
...
try {
    const { data, error } = await supabaseClient.from('viajes_medellin_norte').insert([datosParaGuardar]).select(); // <-- Inserción
    if (error) throw error;
...
async function cargarInformeVehiculos() {
    ...
    try {
     ...
     .from('viajes_medellin_norte')
     .select(`vehiculo_id, vehiculos_medellin_norte (numero_interno), tipo_monitoreo, fecha_monitoreo`);
        if (error) throw error;
        ...
        const vehiculo = viaje.vehiculos_medellin_norte?.numero_interno || 'Desconocido';  
...
async function cargarInformeAdvertencias() {
    ...
    .select(`
        *,
        conductores_medellin_norte (nombre),
        vehiculos_medellin_norte (numero_interno),
        tablas_medellin_norte (nombre_tabla)
    `);
